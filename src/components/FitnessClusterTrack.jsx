...
import {
  ...
  TrendingUp, FileDown, Dumbbell, BicepsFlexed, HeartPulse,
} from 'lucide-react'
...
const TYPE_ICON = { Strength: Dumbbell, Biceps: BicepsFlexed, Triceps: Dumbbell, Cardio: HeartPulse }
const TYPE_COLOR = {
  Strength: 'bg-brand-100 text-brand-700', Biceps: 'bg-sky-100 text-sky-600',
  Triceps: 'bg-violet-100 text-violet-600', Cardio: 'bg-red-100 text-red-600',
}
      ...
      {expanded && (
        <div className="space-y-4 border-t border-slate-100 bg-slate-50/60 p-4">
          ...
          <div>
            ...
            <input className="input h-10 text-sm" value={ex.notes || ''} placeholder="e.g. reduced weight, felt easy, add reps next time…" onChange={onNotesChange} onBlur={onNotesBlur} />
          </div>
...
// Full-screen cluster tracker for a saved fitness plan — every day of the
// plan is a badge in a horizontal cluster, and every prescribed exercise for
// the selected day is an icon tile the trainer (or client, handed the
// device) taps to open, mark done and log real session data. Writes land
// straight on the plan doc (live, best-effort) so nothing needs a "Save" step.
export default function FitnessClusterTrack({ client, plan, plans = [], onClose }) {
  ...
  function reorderExercises(fromIdx, toIdx, exList, dayNum) {
    ...
    const fromCardio = exList[fromIdx]?.type === 'Cardio'
    if ((exList[toIdx]?.type === 'Cardio') !== fromCardio) return // keep main/cardio sections separate
    const next = [...exList]
  ...
  async function persist(nextDays) {
    setSaving(true)
    try { await updateFitnessPlan(client.id, plan.id, { ...plan, days: nextDays }) }
  ...
  async function downloadReport(scope) {
    ...
    try {
      ...
      await generateFitnessReport(client, { plans: source, action: 'download' })
    } catch (_) { /* best-effort */ }
    setReportBusy(false)
  }

  if (showPerf) {
    return <FitnessPerformance client={client} plans={plans.length ? plans : [plan]} onClose={() => setShowPerf(false)} />
  }
      ...
      <div className="mx-auto flex h-full w-full max-w-5xl flex-col overflow-hidden bg-white shadow-2xl sm:h-[calc(100%-2rem)] sm:rounded-3xl">
        ...
        <div className="flex items-center justify-between gap-3 bg-gradient-to-r from-brand-600 to-brand-500 px-4 py-3.5 text-white sm:px-6">
          <div className="min-w-0">
            ...
            <p className="truncate text-lg font-bold">{client.name} <span className="font-normal text-white/70">· {plan.bill?.service || plan.reason || 'Fitness plan'}</span></p>
          </div>
            ...
            {reportMenuOpen && (
              <>
                ...
                <div className="absolute left-0 z-10 mt-1 w-56 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl">
                  ...
                  <button type="button" onClick={() => downloadReport('all')} className="block w-full px-3.5 py-2.5 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50">Full fitness history (all days)</button>
                </div>
              </>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
          {dayData && (
            <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-slate-500">
              ...
              {dayData.home && <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 font-medium text-slate-500"><Home size={11} /> Home workout</span>}
            </div>
          )}
            ...
            <div className="space-y-5">
              ...
              {cardio.length > 0 && (
                <div>
                  ...
                  {cardio.length > 1 && <p className="mb-2 text-[11px] font-medium text-slate-400">Press and hold a tile to drag and reorder.</p>}
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 2xl:grid-cols-3">
                    {cardio.map((ex) => {
                      const idx = exercises.indexOf(ex)
                      const key = `${activeDay}-${idx}`
