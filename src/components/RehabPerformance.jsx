...
// Flattens every day across every one of the client's rehab plans (oldest
...
function recommend(rows) {
  if (!rows.length) return 'No sessions recorded yet — build a rehab plan to start tracking progress here.'
  ...
  if (rate >= 50) return `Moderate consistency — ${rate}% of sessions completed. Consider a reminder call before each session, or simplifying the home program.`
...
export default function RehabPerformance({ client, plans, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-900/50 p-4 backdrop-blur-sm">
      <div className="max-h-[92vh] w-full max-w-3xl animate-pop-in space-y-5 overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl sm:p-6">
        ...
        {rows.length === 0 ? (
          <p className="card py-10 text-center text-sm text-slate-400">No sessions yet — build a rehab plan to start tracking progress here.</p>
        ) : (
