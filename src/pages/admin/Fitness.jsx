...
import {
  watchClients, addFitnessPlan, updateFitnessPlan, watchFitnessPlans, deleteFitnessPlan,
  watchServiceCharges, ensureRehabPackagesSeeded, setAccountingForFitnessPlan, deleteAccountingForFitnessPlan,
  watchFitnessTemplates, addFitnessTemplate, updateFitnessTemplate, deleteFitnessTemplate, updateClient,
} from '../../lib/firestore'
import {
...
} from '../../lib/customFitnessExercises'
import {
...
} from '../../lib/customFitnessTaxonomy'
...
import FitnessPerformance from '../../components/FitnessPerformance'
...
const BUILTIN_TYPES = FITNESS_TYPES
...
function FitnessApp() {
    ...
    return (
      <div className="space-y-5">
        ...
        {showForm ? (
          <ClientForm
            clients={clients}
            defaultPrograms={['W2W Fitness']}
            variant="fitness"
            onCreated={(id) => { setShowForm(false); setParams({ client: id }) }}
            onClose={() => setShowForm(false)}
          />
        ) : showTemplates ? (
          <FitnessTemplateManager onClose={() => setShowTemplates(false)} />
        ) : (
          <FitnessClientPicker clients={clients} onPick={(id) => setParams({ client: id })} onNew={() => setShowForm(true)} onTemplates={() => setShowTemplates(true)} />
        )}
      </div>
    )
  }
  ...
  if (!client) {
    return (
      <div className="space-y-5">
        <AdminPageHeader title="Fitness" />
        <FitnessClientPicker clients={clients} note="That patient could not be found — pick again." onPick={(id) => setParams({ client: id })} onNew={() => setShowForm(true)} />
      </div>
    )
  }
...
function FitnessClientPicker({ clients, onPick, onNew, onTemplates, note }) {
  ...
  // Default to fitness-registered patients only — showing every Treatment-only
    ...
    <div className="space-y-5">
      <div className="card space-y-4 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Tap a patient below to build their fitness plan, or register a new one.</p>
          </div>
        </div>
      ...
      {onTemplates && (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="font-bold text-slate-900">Create Fitness Templates</p>
      ...
      {clients.length === 0 ? (
        ...
        <p className="card py-12 text-center text-sm text-slate-400">
          {showAll ? `No patients match “${q}”.` : fitnessRegisteredClients.length === 0 ? 'No patients registered for W2W Fitness yet.' : `No fitness patients match “${q}”.`}
        </p>
      ) : (
      ...
      {clients.length > 0 && fitnessRegisteredClients.length !== clients.length && (
        <div className="text-center">
          <button type="button" onClick={() => setShowAll((v) => !v)} className="text-sm font-medium text-brand-600 hover:underline">
            {showAll ? 'Show fitness patients only ▲' : 'Not seeing who you need? Show all clients too ▾'}
          </button>
        </div>
      )}
    </div>
...
function CreateRegionWizard({ onSaved, onClose }) {
  ...
  const knownTypes = [...new Set([...FITNESS_TYPES, ...customTypes])]
...
function typesForAnyRegion(region, customTypes) {
  const builtIn = FITNESS_REGIONS.includes(region) ? typesForRegion(region) : FITNESS_TYPES
...
function AddExerciseWidget({ existingNames, onAdd }) {
  ...
  const { isFav: isFavEx, toggle: toggleFavEx, sortWithFavs: sortEx } = useFavorites('fitness_exercise')
  ...
  const regionOptions = sortRegions([...FITNESS_REGIONS, ...customRegions.filter((r) => !FITNESS_REGIONS.includes(r))])
  ...
  const isCustomRegion = (r) => !FITNESS_REGIONS.includes(r)
...
function ExerciseCard({ ex, onChange, onRemove, hideDone }) {
  ...
  const { isFav: isFavProg, toggle: toggleFavProg, sortWithFavs: sortProg } = useFavorites('fitness_progression')
  const [changing, setChanging] = useState(false)
  const [customRegions] = useState(() => getCustomRegions())
  const [customTypes] = useState(() => getCustomTypes())

  const regionOptions = [...FITNESS_REGIONS, ...customRegions.filter((r) => !FITNESS_REGIONS.includes(r))]
      ...
      <div className="flex items-start justify-between gap-2">
        {changing ? (
          <div className="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-3">
            <FavSelect favKey="fitness_region" value={ex.region} options={regionOptions} onChange={changeRegion} placeholder="Region" />
            <FavSelect favKey="fitness_type" value={ex.type} options={typeOptions} onChange={changeType} placeholder="Type" />
            <FavSelect favKey="fitness_exercise_pick" value={ex.name} options={nameOptions} onChange={set('name')} placeholder="Exercise" />
          </div>
        ) : (
          <div>
...
// be reordered within its section (main exercises vs cardio) either by
...
// One day's editor: date, home-workout flag, session-completed flag, and the
// exercise list — always anchored BELOW the Add Exercise widget (cardio
// grouped in its own sub-section).
function DayEditor({ day, allDays, onCopyFromDay, onOpenCrossPatientCopy, onApplyFullTemplate, onChangeDay }) {
  const exercises = day.exercises || []
  const main = exercises.filter((e) => e.type !== 'Cardio')
  ...
  useEffect(() => watchFitnessTemplates(setTemplates), [])
  ...
  // Reorder within a section (main exercises vs cardio never mix) — moves
  ...
  function reorderExercises(fromIdx, toIdx) {
    ...
    const isCardio = exercises[fromIdx]?.type === 'Cardio'
    const sectionIdx = []
    exercises.forEach((e, i) => { if ((e.type === 'Cardio') === isCardio) sectionIdx.push(i) })
  ...
  function moveExercise(idx, direction) {
    const isCardio = exercises[idx]?.type === 'Cardio'
    const sectionIdx = []
    exercises.forEach((e, i) => { if ((e.type === 'Cardio') === isCardio) sectionIdx.push(i) })
  ...
  async function saveAsTemplate() {
    ...
    try { await addFitnessTemplate(n, [{ day: 1, exercises: exercises.map((e) => ({ ...e, done: false, notes: '', progression: [] })) }]) } catch (_) { /* rules may need publishing */ }
    setTemplateName(''); setSavingTemplate(false)
  }
  async function renameTemplate(id) {
    const n = renameVal.trim(); if (!n) return
    try { await updateFitnessTemplate(id, { name: n }) } catch (_) { /* rules may need publishing */ }
    setRenamingId(null)
  }
  async function removeTemplate(id) {
    if (!window.confirm('Delete this template?')) return
    try { await deleteFitnessTemplate(id) } catch (_) { /* rules may need publishing */ }
  }
...
function FitnessPlanner({ client, clients = [], editId = '', onChangeClient, navigate }) {
  ...
  // patient's fitness history straight into the active day of this plan.
  ...
  async function save(e) {
    ...
    if (!form.therapist) {
      ...
      requestAnimationFrame(() => document.getElementById('fitness-therapist')?.scrollIntoView({ behavior: 'smooth', block: 'center' }))
      return
    }
    setBusy(true)
    try {
      ...
      // A patient who's just been given a fitness plan is, by definition, now
      // registered for Fitness too — tag it automatically (best-effort).
      try {
        if (!Array.isArray(client.programs) || !client.programs.includes('W2W Fitness')) {
          await updateClient(client.id, { programs: [...(Array.isArray(client.programs) ? client.programs : []), 'W2W Fitness'] })
        }
      } catch (_) { /* best-effort */ }
  ...
  async function removePlan(p) {
    if (!window.confirm(`Delete this ${p.totalDays || p.days?.length || ''}-day fitness plan? This cannot be undone.`)) return
    await deleteFitnessPlan(client.id, p.id)
    try { await deleteAccountingForFitnessPlan(p.id) } catch (_) { /* best-effort */ }
  }
  ...
  // the Previous fitness plans list.
  async function markPlanComplete(p) {
    ...
    await updateFitnessPlan(client.id, p.id, { ...p, days })
  }
      ...
      <form onSubmit={save} className="space-y-5">
        ...
        <div className="card space-y-4 p-5 md:p-6">
              ...
              {Array.isArray(client.fitnessGoals) && client.fitnessGoals.length > 0 && (
                <div className="mt-1.5 flex flex-wrap gap-1">
                  {client.fitnessGoals.map((g) => (
                    <span key={g} className="rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold text-brand-700 ring-1 ring-brand-200">{g}</span>
                  ))}
                </div>
              )}
            </div>
                ...
                {activePlans.map((p) => {
                  ...
                  return (
                    <div key={p.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white p-3 shadow-sm">
                      <div>
                        <p className="text-sm font-bold text-slate-900">{p.bill?.service || 'Fitness plan'} · Day {Math.min(done + 1, total)} of {total}</p>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="label text-sm">Fitness goal</label>
          ...
          {billOpen && (
            <div className="space-y-3 p-4 sm:p-5">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div><label className="label text-sm">Mode</label><FavSelect favKey="fitness_pay_mode" value={form.bill.mode} options={PAY_MODES} onChange={(m) => { setForm((f) => ({ ...f, bill: { ...f.bill, mode: m } })); setDirty(true) }} /></div>
              </div>
      ...
      {plans.length > 0 && (
        <div className="card p-5">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <h3 className="text-base font-bold text-brand-700">Previous fitness plans</h3>
...
// Clone a specific day's exercises from ANY other patient's fitness history
...
function CopyFromPatientModal({ clients, currentClientId, onApply, onClose }) {
  ...
  useEffect(() => {
    ...
    return watchFitnessPlans(pickedClientId, setPlans)
  }, [pickedClientId])
      ...
      <div className="max-h-[92vh] w-full max-w-lg animate-pop-in space-y-4 overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl">
        ...
        {pickedClientId && (
          <div>
            ...
            {plans.length === 0 ? (
              <p className="text-sm text-slate-400">This patient has no fitness plans yet.</p>
            ) : (
...
// "Create Fitness Templates" entry on the Fitness home page. Saves into the same
// top-level `fitnessTemplates` collection DayEditor's "Save this day as
...
function FitnessTemplateManager({ onClose }) {
  const [templates, setTemplates] = useState([])
  ...
  async function removeTemplate(t) {
    ...
    try { await deleteFitnessTemplate(t.id) } catch (_) { /* rules may need publishing */ }
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (form.id) await updateFitnessTemplate(form.id, { name, days })
      else await addFitnessTemplate(name, days)
      setForm(null)
    } catch (err) {
      console.error('save fitness template failed:', err)
      ...
      <div className="card space-y-4 p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-brand-50 text-brand-600"><LayoutTemplate size={22} /></div>
            <div>
              <h2 className="font-bold text-slate-900">Fitness templates</h2>
