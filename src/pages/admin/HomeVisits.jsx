...
export default function HomeVisits() {
    ...
    return (
      <div className="space-y-5">
        ...
        {showForm ? (
          ...
          <HomeVisitClientPicker clients={clients} onPick={(id) => setParams({ client: id })} onNew={() => setShowForm(true)} />
        )}
      </div>
    )
  }
  ...
  if (!client) {
    return (
      <div className="space-y-5">
        <AdminPageHeader title="Home Visits" />
        <HomeVisitClientPicker clients={clients} note="That patient could not be found — pick again." onPick={(id) => setParams({ client: id })} onNew={() => setShowForm(true)} />
      </div>
    )
  }
...
function HomeVisitClientPicker({ clients, onPick, onNew, note }) {
  ...
  // clinic patient needs a home visit added for the first time.
    ...
    <div className="space-y-5">
      ...
      {clients.length === 0 ? (
        ...
        <p className="card py-12 text-center text-sm text-slate-400">
          {showAll ? `No patients match “${q}”.` : hvClients.length === 0 ? 'No patients registered for Home Visits yet.' : `No home visit patients match “${q}”.`}
        </p>
      ) : (
...
function HomeVisitForm({ client, editId = '', onChangeClient, navigate }) {
      ...
      <div className="card space-y-4 p-5 md:p-6">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            ...
            <TherapistSelect id="hv-therapist" invalid={therapistInvalid} value={therapist} onChange={(v) => { setTherapist(v); touch(); setTherapistInvalid(false) }} />
          </div>
